  ...
  private
    ...
    def result(positions)
      if positions[0] == positions[1] && positions[1] == positions[2]
        return "Player #{positions[0]} has won!"
      end
      if positions[3] == positions[4] && positions[4] == positions[5]
        return "Player #{positions[3]} has won!"
      end
      ...
      if positions[1] == positions[4] && positions[4] == positions[7]
        return "Player #{positions[1]} has won!"
      end
      ...
      if positions[2] == positions[4] && positions[4] == positions[6]
        return "Player #{positions[2]} has won!"
      end
      if !positions.any? (Integer)
        return "This match ends with a tie"
      end
    end
  ...
  public
    def game
      ...
      while i < 9 do
        ...
        table(positions)
        puts ""
        if result(positions).instance_of?(String)
          return puts result(positions)
        end
      end
    end
end
